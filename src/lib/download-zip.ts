...
export function exportDownloadFilenameFromUrl(
  url: string,
  fallbackId: string,
): string {
  const segment = url.split("/").pop()?.split("?")[0]?.trim();
  return segment && segment.length > 0 ? segment : `${fallbackId}.mp4`;
}
...
export function resolveDownloadFolderName(
  ...
  return (
    requested?.replace(/[^\w.-]/g, "_").trim() ||
    createDownloadFolderName(itemCount === 1 ? "hookr-export" : "hookr-exports")
  );
}

export async function buildZipDownloadResponse(

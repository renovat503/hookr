    ...
    try {
      ...
      if (mode === "edit" && editingPost) {
        const res = await fetch(
          `/api/instagram/schedule/${encodeURIComponent(editingPost.id)}`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ scheduledAt, caption }),
          },
        );
        ...
        <div className="max-h-[70dvh] space-y-4 overflow-y-auto p-4">
          ...
          {mode === "create" ? (
            ...
            <p className="text-sm text-muted">
              @{accounts.find((a) => a.id === editingPost.accountId)?.username}
              {" · "}
          ...
          <label className="block space-y-1.5">
            ...
            <textarea
              ...
              placeholder="Optional caption for Instagram"
            />
          </label>
        </div>

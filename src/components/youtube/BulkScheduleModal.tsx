...
function defaultDescriptionStorageKey(accountId: string) {
  return `hookr-bulk-description-${accountId}`;
}

function descriptionForExport(
  exp: LibraryExport,
  defaultDescription: string,
  descriptions: Record<string, string>,
  exportId: string,
) {
  if (exportId in descriptions) return descriptions[exportId] ?? "";
  return resolveScheduleDescription(exp, null, defaultDescription);
}
...
export function BulkScheduleModal({
    ...
    if (accountId) {
      try {
        setDefaultDescription(
          localStorage.getItem(defaultDescriptionStorageKey(accountId)) ?? "",
        );
      } catch {
        setDefaultDescription("");
      }
  ...
  const persistDefaultDescription = (value: string) => {
    ...
    try {
      if (value.trim()) {
        localStorage.setItem(defaultDescriptionStorageKey(accountId), value);
      } else {
        localStorage.removeItem(defaultDescriptionStorageKey(accountId));
      }
    } catch {
  ...
  const applyDefaultToSelected = (value: string, onlyUncustomized = true) => {
    setDescriptions((current) => {
      ...
      for (const exportId of selected) {
        ...
        next[exportId] = exp
          ? resolveScheduleDescription(exp, null, value)
          : value.trim();
      }
      return next;
    });
  ...
  const toggle = (exportId: string) => {
    setSelected((current) => {
      ...
      if (exp) {
        setDescriptions((prev) => ({
          ...prev,
          [exportId]: resolveScheduleDescription(exp, null, defaultDescription),
        }));
      }
    ...
    setDescriptions(
      Object.fromEntries(
        ids.map((id) => {
          ...
          return [id, exp ? resolveScheduleDescription(exp, null, defaultDescription) : ""];
        }),
      ),
    );
    setCustomized(new Set());
  ...
  const updateVideoDescription = (exportId: string, value: string) => {
    setDescriptions((prev) => ({ ...prev, [exportId]: value }));
    setCustomized((prev) => new Set(prev).add(exportId));
  };

  const resetVideoDescription = (exportId: string) => {
    const exp = exportById.get(exportId);
    if (!exp) return;
    setDescriptions((prev) => ({
      ...prev,
  ...
  const confirm = async () => {
    ...
    try {
      ...
      await onConfirm(assignments, defaultDescription);
      persistDefaultDescription(defaultDescription);
      setSelected([]);
      setDescriptions({});
      setCustomized(new Set());
      onClose();
        ...
        <div className="space-y-4 overflow-y-auto p-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-2">
              <label
                htmlFor="bulk-default-description"
              ...
              {selected.length > 0 ? (
                <button
                  type="button"
                  onClick={() => applyDefaultToSelected(defaultDescription, false)}
            ...
            <textarea
              id="bulk-default-description"
              value={defaultDescription}
              onChange={(e) => handleDefaultDescriptionChange(e.target.value)}
              onBlur={() => persistDefaultDescription(defaultDescription)}
              rows={3}
                        ...
                        <div className="mt-2 space-y-1.5 pl-7">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {isCustomized ? (
                              <button
                                type="button"
                                onClick={() => resetVideoDescription(exp.id)}
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              updateVideoDescription(exp.id, e.target.value)
                            }
                            rows={2}
                            placeholder="YouTube description for this video"

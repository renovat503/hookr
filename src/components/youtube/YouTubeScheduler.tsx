...
import {
  ScheduleCalendar,
} from "@/components/youtube/ScheduleCalendar";
import { SchedulePostModal } from "@/components/youtube/SchedulePostModal";
import { ScheduleWeekGrid } from "@/components/youtube/ScheduleWeekGrid";
...
type QueueItem = YouTubeScheduledPost & { exportUrl?: string | null };
...
export function YouTubeScheduler() {
  ...
  const [editingPost, setEditingPost] = useState<
    (YouTubeScheduledPost & { exportUrl?: string | null }) | null
  >(null);
  ...
  const openEditModal = (post: YouTubeScheduledPost & { exportUrl?: string | null }) => {
    setModalMode("edit");
    setEditingPost(post);
  ...
  const openSlotModal = (slot: ScheduleSlot, post: YouTubeScheduledPost | null) => {
    if (post) {
      openEditModal(post);
      return;
    }
  ...
  const patchScheduledAt = async (postId: string, scheduledAt: string) => {
    const res = await fetch(
      `/api/youtube/schedule/${encodeURIComponent(postId)}`,
      {
        method: "PATCH",
  ...
  const removeFromQueue = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/youtube/queue?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
      const json = (await res.json()) as { error?: string };
  ...
  const bulkSchedule = async (assignments: Array<{
    ...
    setError(null);
    try {
      const res = await fetch("/api/youtube/schedule/bulk", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const json = (await res.json()) as {
        error?: string;
        scheduled?: YouTubeScheduledPost[];
    ...
    return (
      <div className="space-y-4 rounded-2xl border border-border bg-surface/70 p-6 text-center">
        {error ? (
          ...
          <p className="text-sm text-muted">Could not load YouTube.</p>
        )}
        <button
          type="button"
          onClick={() => void load()}
    ...
    return (
      <div className="rounded-2xl border border-border bg-surface/70 p-6 text-center">
        <p className="text-sm text-muted">
          Add YouTube credentials to your environment, then connect an account.
        </p>
      </div>
    );
  }
      ...
      {data.accounts.length === 0 ? (
        ...
        <>
          {activeAccount ? (
            <PostingGoalPanel
              accountId={activeAccountId}
              channelTitle={activeAccount.channelTitle}
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <section className="rounded-2xl border border-border bg-surface/70 p-4">
            <h3 className="mb-3 text-sm font-semibold text-muted">
              {activeAccount ? `${activeAccount.channelTitle} queue` : "Queue"}
            </h3>
